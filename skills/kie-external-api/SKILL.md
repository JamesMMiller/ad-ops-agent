...
4. **Offer to stitch** locally (no API) via **`edit-video`**:
   - Prefer soft-stitch (crossfade) for lookbooks / premium reveals — see [`shared/skills/edit-video/SKILL.md`](../../shared/skills/edit-video/SKILL.md).
   - Helper: `./shared/skills/edit-video/scripts/soft-stitch.sh --fade 0.6 --out final.mp4 clip-a.mp4 clip-b.mp4`
   - Hard `ffmpeg concat -c copy` only if the user explicitly wants a hard cut.
