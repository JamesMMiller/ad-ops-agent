...
| `input.aspect_ratio` | no | `"1:1"`, `"9:16"`, `"16:9"` (and others per marketplace) | Default `9:16` for Reels/Stories; use **`1:1`** for Meta feed creatives that must run on both Instagram and Facebook. |
...
## Multi-clip stitch (when the ad is longer than 15s)

Seedance 2.0 maxes at **15s per job**. For longer ads, generate segments then join **locally**:

1. Prefer **soft-stitch** (crossfade) via [`shared/skills/edit-video/SKILL.md`](../../../../shared/skills/edit-video/SKILL.md) — default for lookbooks / dark-void reveals.
2. Do **not** default to hard `ffmpeg concat -c copy` (abrupt A/V jump).
3. Helper: `./shared/skills/edit-video/scripts/soft-stitch.sh --fade 0.6 --out final.mp4 clip-a.mp4 clip-b.mp4`
...
- [ ] **Aspect ratio** — default **`1:1`** for Meta IG+FB feed dual-placement; `9:16` for Reels/Stories-only; `16:9` for landscape
