...
- **Cursor:** `.cursor/hooks.json` runs `sessionStart` (sync skills + context banner). Rules in `.cursor/rules/`. Skills discoverable at `.cursor/skills/` after `./scripts/sync-skill.sh` or a new agent session.
- **Skills:**
