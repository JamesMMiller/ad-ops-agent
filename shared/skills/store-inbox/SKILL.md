...
Sender
  ...
  → Apps Script (LLM classify)
        ...
        ├─ OWNER ASK → pitch? decline+deal · else ask what it concerns (no escalate yet); escalate on clarifying reply
...
6. Test: pitch (deal or decline), “Do you ship to France?” (FAQ UK-only), “Speak to the owner about SEO” (pitch+deal), “Speak to the owner?” (ask topic only — no escalate), then clarifying reply (escalate), “Any discount?” (DEAL), “Where is my order?” (escalate).
